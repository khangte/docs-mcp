...
`get_endpoint_details` MCP 도구가 쓰는 조회 경로다. 다음 두 가지를 보장한다.
...
class EndpointDetailsService:
    ...
    def get_details(
        ...
        return _to_details(endpoint, example_code)


def _to_details(endpoint: ApiEndpoint, example_code: str | None) -> EndpointDetailsResult:
    ...
    return EndpointDetailsResult(
        ...
        request_body=(
            _to_request_body(endpoint.request_body)
