...
from app.services.indexer.embedding_provider import cosine_similarity
...
class InMemoryVectorIndex:
    ...
    def __init__(self) -> None:
        """청크ID→벡터 저장용 빈 dict 와 RLock 을 초기화한다."""
    ...
    def upsert_many(self, pairs: list[tuple[str, list[float]]]) -> None:
        """여러 청크 벡터를 한 번의 락 획득으로 일괄 추가/갱신한다."""
        with self._lock:
            for chunk_id, vector in pairs:
                self._vectors[chunk_id] = list(vector)

    def replace_all(self, pairs: list[tuple[str, list[float]]]) -> None:
        """인덱스 전체를 새 데이터로 원자적으로 교체한다."""
    ...
    def search(
        ...
        락 안에서 snapshot 을 복사한 뒤 락 밖에서 코사인 연산을 수행해
        쓰기 스레드를 불필요하게 블로킹하지 않는다.
        """
        if top_k <= 0:
            return []
        with self._lock:
