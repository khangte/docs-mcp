...
- **단일 스토어 하이브리드 유지**: FTS5 와 sqlite-vec 가 **같은 파일·같은 커넥션**에 있어, RRF 융합(FTS5 MATCH 등수 + vec KNN 등수)을 한 곳에서 수행 가능. `07_search_rrf_reevaluation.md` 가 기댄 **"두 랭커가 한 스토어"** 전제가 pgvector 와 동일하게 보존된다(Qdrant 분리와 대조적으로 여기선 안 깨진다).
