...
>
> **갱신 이력 (2026-08-27)**: `docs/architect-review/62_meta_filter_owner_created_folder_shared_design.md`
> 의 Stage A(owner/createdTime 필터, 노출 포함)와 Stage B(folder_ids 필터)가 구현·리뷰 완료됐다.
> Stage C(sharedWith)는 C0 probe 결과 옵션 A·B 모두 반려되어 이번 범위에서 종료했다
> (`docs/architect-review/63_shared_with_c0_gate_verdict.md`). 2절(Metadata filtering /
> Permission·access control), 9절(항목 3·12)을 갱신했다. 항목12(권한 전제)는 Stage C 연장이 아니라
> 호출자 신원·실효 권한을 함께 다루는 별도 보안 트랙으로 재착수한다(63번 4절 게이트).
>
...
| Metadata filtering        | `project`, `source` + `modified_after`/`modified_before`/`mime_types`/**`owner`/`created_after`/`created_before`/`folder_ids`** hard filter(3 arm 전부 SQL 적용) | createdTime/modifiedTime/mimeType/folderId/owner/sharedWith 로 hard filter | 날짜·mimeType·owner·createdTime·folderId 는 해소. sharedWith 만 probe 결과 수집 불가로 미구현(범위 종료) | 상       | **날짜·mime·owner·folder 구현 완료 (2026-08-26/27)** / sharedWith 는 종료(62·63번) |
...
| Permission/access control | **없음**. 서비스 계정 1개가 본 것 = 모든 MCP 호출자가 검색 가능. 최종 사용자 신원 개념 자체가 없음                                                                | owner/sharedWith 를 필터·시그널로 활용                                     | `owner` 필터·노출은 해소(Stage A). `sharedWith` 는 C0 probe 결과 서비스 계정=뷰어 전제에서 ACL 수집 자체가 불가해 종료. 권한 모델 부재는 여전(62·63번)                        | 상(보안) | **owner 구현 완료 (2026-08-27) / sharedWith 종료, 항목12는 별도 보안 트랙** |
...
| 3   | Metadata filtering          | `project`/`source` + `modified_after`/`modified_before`/`mime_types`/`owner`/`created_after`/`created_before`/`folder_ids` hard filter | createdTime/modifiedTime/mimeType/folderId/owner/sharedWith hard filter | **날짜·mimeType·owner·createdTime·folderId 구현 완료 (2026-08-26/27)** — 개선 #2, 62번 Stage A·B 로 반영. `sharedWith` 는 C0 probe 결과 수집 불가로 종료했다(63번) |
...
| 12  | Permission / access control | 없음. 서비스 계정이 본 것 = 모든 호출자가 검색 가능                                        | owner/sharedWith 를 필터·시그널로 활용                                  | **owner 는 구현 완료(Stage A), sharedWith 는 C0 게이트에서 종료** — probe 결과 서비스 계정=뷰어 전제에서 `permissions` 수집 자체가 불가(옵션 A 반려), 문서별 `permissions.list` 호출도 동기화 비용·ACL PII·호출자 신원 부재로 반려(옵션 B 반려, 63번). 권한 모델 자체는 여전히 부재 — Stage C 연장이 아니라 호출자 신원·실효 권한을 함께 다루는 별도 보안 트랙으로 재착수(63번 4절 게이트) |
...
2026-08-26 구현 완료**(5.3절 파생 항목은 개선 #4 의 T1 로 함께 해소). **3 은 owner/createdTime
(Stage A)·folder_ids(Stage B) 로 2026-08-27 추가 구현 완료**(62번). **12 는 owner 노출까지는
Stage A 로 해소됐으나 sharedWith 는 C0 probe 게이트에서 종료했다**(63번) — 남은 채택분은
1(intent parsing 계열)뿐이고, 12(권한 모델 자체)는 별도 보안 트랙 재착수 게이트가 선행돼야 한다.
