...
| B    | 문서마다 `permissions.list` 호출                               | **문서 수 N 만큼 호출 증가** — 지금까지 폴더 수에 비례하던 동기화가 문서 수에 비례하게 된다. rate limit 위험이 질적으로 달라진다 | 통과 못 하면 이것뿐이며, 권고하지 않음 |
...
C (게이트)
└─ C0 probe → 결과에 따라 옵션 A/B 판단 → 항목12 와 통합 착수
```
...
3. `sharedWith` = **항목12 로 이관**, 이번 범위에서는 C0 probe 만.
