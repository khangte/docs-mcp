...
- 일시: 2026-08-15
- 작성: architect
- 선행: `docs/architect-review/40_doc36_step13_legacy_fetch_removal_gate.md` §4,
  `docs/architect-review/41_body_index_backfill_gate_fix.md`,
  `docs/architect-review/42_body_backfill_content_normalization_and_commit_boundary.md`
...
카운터 자체는 지금 계산이 맞다. 커밋 경계(doc42 §3) 관점에서는 빈 본문 경로도 세는 게
...
맞춘다. 카운터를 둘로 쪼개지 않는다(보고용 색인 건수는 SQL 로 보는 것이 doc41 §6 판단이고
그대로 유효하다).
...
→ **텍스트가 있는 문서의 색인률 100%(136/136).** 미설명 잔여분은 0건이다. doc40 §4(b) 가
말한 "색인률 목표치"는 이 정의로 충족됐다. 전체 대비 49.6% 라는 숫자는 분모에 애초에
색인 불가능한 바이너리가 들어간 값이라 게이트 지표로 쓰면 안 된다.

### 완전 색인이 전제조건이 아니라는 판단은 맞다 — 그리고 더 강하다

doc39 §2.3 대로 title arm 은 `deterministic_document_id` 를 순수 계산하므로 미색인 문서도
...
- 코드 삭제는 하지 않는다. `DOCS_MCP_DOCUMENT_SEARCH_STRATEGY` 기본값만
  ...
  되돌린다(doc39 §2.7 degrade 규약 유지).
- 겉면 계약 변경은 이미 명시돼 있다: `score` 스케일이 RRF 로 바뀌어 절대값 비교 불가·
  순서만 유의미(doc39 §2.5), 스니펫 출처가 동기화 시점 캐시가 될 수 있어
  `snippet_as_of` 노출(doc36 §0-2). 추가 계약 변경 없다.
- doc36 13번(구경로 삭제)은 이 전환 후 실사용 확인까지 계속 보류다(doc40 §4(c)).

## 3. 남는 비용 — 매 실행 138건 재fetch (지금은 고치지 않음)

doc41 의 자기 치유 게이트(`document_id is None` 이면 재시도)에는 실패 기억이 없다. 그래서
...
3. 실사용에서 검색 품질 확인 → 이상 없으면 doc36 13번(플래그·fetch 브랜치 동시 삭제) 착수.
