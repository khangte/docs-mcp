...
인셋 "boundary exceptions" 첫 번째 항목의 서브 라벨을 `get_endpoint_details 내부`라고
적었으나, 실제로 `DocumentRepository`를 직접 생성하는 코드는 `get_endpoint_details`가
아니라 `get_raw_document` 안에 있다.
...
`DocumentRepository` 직접 생성은 195행이며, 이는 190행에서 시작하는 `get_raw_document`
본문에 속한다. `get_endpoint_details`(90~126행) 범위 밖이다. **지적은 정확하다.**

### 0.3 판정 및 조치

지적을 수용한다. 다이어그램의 다른 부분(의존 방향, 계층 구성, 두 번째 예외 항목)은
영향을 받지 않는다 — 잘못된 것은 예외가 발생한 **함수 이름 하나**이고, "MCP 계층이
서비스 계층을 건너뛰고 리포지토리를 직접 만든다"는 예외 자체의 성립 여부는 그대로다.
...
2. 부수 정정: `tools/endpoints` 박스 서브 라벨 `4 tools · API 명세 질의` →
   `4 tools + 1 resource`(인셋이 `@mcp.resource`를 지목하게 된 이상 다이어그램 본체에
   resource 등록이 없는 것처럼 보이면 두 표기가 서로 모순되므로 맞춤)

이 상태(예외 1행: `get_raw_document` 항목, 예외 2행: 계층 경계 미판정)에서 아래 §1~§2
판정이 이어진다.
...
즉 기존 문서의 17은 tool 16 + resource 1을 합산한 수치로 보인다. 틀린 수는 아니지만
라벨이 `MCP 도구`라 tool 만 세는 것으로 읽힌다. 신규 문서는 `16 MCP 도구 (4·4·8)`로
tool 만 세고, resource 는 `tools/endpoints` 박스에 별도로 표기하는 방식을 택했다.

기존 `portfolio-architecture.html` 수정 여부는 lead 판단 사항이다 — 선택지는 (a) 그대로
두기, (b) 라벨을 `MCP 도구 + 리소스`로 바꾸기, (c) `16`으로 낮추고 resource 를 별도 표기.
신규 문서와의 정합만 놓고 보면 (b)를 권한다.

### 0.5 검증 결과 (reviewer 보고 기준, 재실행 없음)
...
이 모듈은 doc/31 §4의 결정으로 **MCP 계층에서 서비스 계층으로 내려온** 코드다. 당시
...
2. **되돌리면 더 나빠진다.** 타입을 서비스 모듈로 옮기고 `app/mcp/types.py` 가 그것을
   ...
   정의되는 편이 맞다.
3. **선행 결정과 일치.** doc/31 §4가 "반환 스키마 무변경"을 이동 조건으로 명시했고,
   그 조건은 지금도 유효하다.
