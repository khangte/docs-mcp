...
### P1 — 키워드 검색을 Postgres FTS(tsvector + GIN)로 이관 ★핵심
...
### P2 — 키워드 경로에서 embedding 컬럼 로딩 제거 (quick win)
...
### P3 — 문서 검색 2단계 본문 fetch 병렬화 ★문서 검색 지연 핵심
...
### P4 — document_meta 1단계 ILIKE를 pg_trgm GIN 인덱스로
...
### P6 — HNSW `ef_search` 튜닝 / `candidate IN` 후처리 재검토
