...
- 상태: **분석/제안 only** (코드 미수정)
- 일시: 2026-08-11
- 작성: architect
- 선행 문서: `docs/search-performance-improvements.md`(P1~P6·RRF 전부 구현완료), `docs/search-flow.md`(현재 흐름)
...
| **S3** | 문서 서술 정정 | `search-flow.md`/`endpoint_candidate_search` docstring의 "항상 병렬 실행" → 현행 직렬 반영 (S1 착수 전이라면) | — | 낮음 | 문서-코드 정합성. **살아있는 문서 규칙상 별건이라도 정정 필요** |
