...
- 작성: architect
- 근거 설계: `docs/search-quality-post-rrf.md` P1
...
- **실패 taxonomy 균형 배분**(각 카테고리 최소 8~12질의). 카테고리는
  `docs/search-rrf-reevaluation.md` 1절 손실 패턴에서 파생:
...
3. **평가 스크립트 실측 재실행**:
   ...
   - 이 실행 결과(지표 표)를 `docs/search-quality-post-rrf.md` P1 절 하단 또는 별도
...
- 커밋 C(선택): `docs: 확장 평가셋 실측 결과 기록`(search-quality-post-rrf.md)
