...
### 3-2. 의존 방향
`api / mcp` → `services` → `repositories` → `models`
...
1. `search_api_docs`: 자연어 질의 기반 API 검색 (Hybrid)
2. `get_endpoint_detail`: 특정 API의 상세 명세(파라미터, 스키마 등) 조회
3. `generate_request_example`: 스키마 기반 코드 예시(curl, python 등) 생성
