"""DocumentSearchService 단위 테스트 (SPEC 기능 7, 8).

핵심 검증 대상은 "2단계 후보 압축"이 실제로 외부 API 호출을 줄이는지다.
페이크 소스의 `fetch_call_count` 로 다음을 단언한다.
...
@pytest.fixture()
def search_service(db_session, fake_document_sources):
    """페이크 소스를 주입한 DocumentSearchService."""
    return DocumentSearchService(
        meta_repo=DocumentMetaRepository(db_session),
        sources=fake_document_sources,
    )
...
def test_no_candidate_never_touches_source(db_session) -> None:
    """후보가 없으면 소스 어댑터 자체가 호출되지 않는다(폭발 페이크로 단언)."""
    _seed_meta(db_session, SOURCE_DRIVE, "d1", "배포 운영 가이드")
    service = DocumentSearchService(
        meta_repo=DocumentMetaRepository(db_session),
        sources={SOURCE_DRIVE: ExplodingDocumentSource(SOURCE_DRIVE)},
    )
...
def test_get_document_unconfigured_source_raises_integration_error(db_session) -> None:
