"""`ProjectDriveSourceRepository` 단위 테스트 (SPEC 기능 4 검증 기준, 339~348행).

`upsert`/`get`/`list_all`/`delete` 공통 계약을 Drive 쪽에서 직접 검증한다.
"""

from __future__ import annotations

from app.repositories.project_source_repository import ProjectDriveSourceRepository


def test_upsert_creates_new_row(db_session) -> None:
    """등록되지 않은 project 에 upsert 하면 새 행이 생긴다."""
    repo = ProjectDriveSourceRepository(db_session)

    row = repo.upsert("A", "folder-a")
    db_session.commit()
...
def test_upsert_same_project_replaces_value_not_new_row(db_session) -> None:
    ...
    repo = ProjectDriveSourceRepository(db_session)
    repo.upsert("A", "folder-a")
    db_session.commit()

    repo.upsert("A", "folder-a-v2")
    db_session.commit()

    rows = repo.list_all()
    assert len(rows) == 1
    assert rows[0].folder_id == "folder-a-v2"
...
def test_list_all_returns_project_ascending_order(db_session) -> None:
    ...
    for project in ("C", "A", "B"):
        repo.upsert(project, f"folder-{project.lower()}")
    db_session.commit()
...
def test_drive_and_notion_registration_are_independent(db_session) -> None:
    """한 프로젝트가 Drive 만 등록해도 정상 동작한다(Notion 저장소와 서로 독립)."""
    repo = ProjectDriveSourceRepository(db_session)

    repo.upsert("drive-only", "folder-x")
    db_session.commit()

    assert repo.get("drive-only") is not None
